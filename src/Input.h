#ifndef INPUT_H
#define INPUT_H

#include <glad/glad.h>
#include <GLFW/glfw3.h>
#include <map>
#include <iostream>

class Input {
public:
	Input();
	~Input();

	void ProcessInput(GLFWwindow* window);
	unsigned char WaitForInput(GLFWwindow* window);
	bool WasPressed(GLFWwindow* window, unsigned char regVal);
	bool* pressed;

private:
	const std::map<unsigned short, unsigned char> keyboardMap = { 
		{GLFW_KEY_1, 0x1}, {GLFW_KEY_2, 0x2}, {GLFW_KEY_3, 0x3}, {GLFW_KEY_4, 0xC},
		{GLFW_KEY_Q, 0x4}, {GLFW_KEY_W, 0x5}, {GLFW_KEY_E, 0x6}, {GLFW_KEY_R, 0xD},
		{GLFW_KEY_A, 0x7}, {GLFW_KEY_S, 0x8}, {GLFW_KEY_D, 0x9}, {GLFW_KEY_F, 0xE},
		{GLFW_KEY_Z, 0xA}, {GLFW_KEY_X, 0x0}, {GLFW_KEY_C, 0xB}, {GLFW_KEY_V, 0xF},
	};
	
};




#endif
